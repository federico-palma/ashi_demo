...
function renderProducts(filter) {
  ...
  grid.querySelectorAll('.play-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const product = PRODUCTS.find(p => p.id === id);
      if (product) showToast(`▶ PLAYING: ${product.title}`, 'accent');
    });
  });
}
...
function showToast(message, color = 'accent') {
  ...
  toast.innerHTML = `<span class="toast-dot" style="background:var(--${color})"></span>${message}`;
...
function animateWaveform(container) {
  ...
  setInterval(() => {
    ...
    bars.forEach((bar, i) => {
      ...
      bar.style.height = h + '%';
      const bright = h > 60 ? 'rgba(246,89,33,0.85)' : 'rgba(246,89,33,0.25)';
...
function initCartButton() {
  ...
  btn.addEventListener('click', () => {
    if (cartCount === 0) {
      showToast('CART IS EMPTY', 'accent');
    } else {
      showToast(`${cartCount} ITEM${cartCount > 1 ? 'S' : ''} IN CART`, 'accent');
    }
  });
}
