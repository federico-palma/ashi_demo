/* ══════════════════════════════════════════════════════════════
   ASHI — DEMO 3 SCRIPT
   Warm, analog waveform colors (ochre VU meter palette)
   ══════════════════════════════════════════════════════════════ */
      ...
      <article class="product-card" data-id="${p.id}" data-type="${p.type}">
        ...
        <div class="product-thumb-wrap">
          <img
            ...
            loading="lazy"
          />
          <span class="product-type-badge ${BADGE_CLASS[p.type]}">${p.type.charAt(0).toUpperCase() + p.type.slice(1)}</span>
...
/* ══════════════════════════════════════════════════════════════
   WAVEFORM BARS — ochre VU meter aesthetic
   ══════════════════════════════════════════════════════════════ */
...
function animateWaveform(container) {
  ...
  setInterval(() => {
    tick++;
    bars.forEach((bar, i) => {
      const phase = (i / bars.length) * Math.PI * 4 + tick * 0.04; /* slower — analog */
      const h = Math.max(6, (Math.sin(phase) * 0.5 + 0.5) * 100);
      bar.style.height = h + '%';
      /* VU meter: ochre low → terracotta peak — like a vintage level meter */
      const bright = h > 70
        ? 'rgba(226, 114, 91, 0.75)'   /* terracotta peak */
        : h > 40
          ? 'rgba(212, 163, 115, 0.65)' /* ochre mid */
          : 'rgba(212, 163, 115, 0.2)'; /* faint ochre base */
      bar.style.background = bright;
    });
  }, 90); /* slightly slower tick — mellow pace */
}
